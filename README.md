# SKY-PANEL (Mikrotik WireGuard Panel)
...
the [Releases](https://github.com/SKY-PANEL/SKY-PANEL/releases/latest)
page.
...
```bash
curl -fSLo mwp https://github.com/SKY-PANEL/SKY-PANEL/releases/latest/download/mwp.linux.$(uname -m)
...
```bash
curl -fSLo mwp https://github.com/SKY-PANEL/SKY-PANEL/releases/latest/download/mwp.darwin.$(uname -m)
sudo install -v -o root -g root -m 755 mwp /usr/local/bin/mwp
rm -f mwp
mwp
```
...
Download the latest binary release from [here](https://github.com/SKY-PANEL/SKY-PANEL/releases/latest) and run it by
...
Run the following command to start SKY-PANEL using Docker:
...
Run the following command to start SKY-PANEL using Docker Compose:
...
```bash
git clone https://github.com/sky-panel/sky-panel.git
cd mwp
...
SKY-PANEL is configured using environment variables. Create a `.env` file in the root project directory (for Docker) or in the
...
Created by [SKY-PANEL](https://github.com/sky-panel) – feel free to reach out via GitHub issues or pull requests.